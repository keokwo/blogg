...
export function TableOfContents({ className, items }: { className?: string; items: TableOfContentsProps[] }) {
  ...
  return (
    <div className={twJoin('not-prose', className)}>
      <aside className="xl:sticky xl:top-[5rem] xl:-mr-6 xl:h-[calc(100vh-4.75rem)] xl:flex-none xl:overflow-y-auto xl:py-16 xl:pr-6">
        <nav aria-labelledby="on-this-page-title" className="w-56">
          ...
          >
            <>
              <Heading level={2} className="text-base lg:text-lg font-semibold leading-7 mb-6 text-dark dark:text-light">
                On this page
              </Heading>
              {items.length > 0 && (
...
function TocLink({ item, activeId }: { item: TableOfContentsProps; activeId: string | null }) {
  return (
    <li key={item.title}>
      <Link
        className={cn(
          'outline-none block no-underline lg:text-[0.885rem]',
          item.url.split('#')[1] === activeId ? 'text-dark dark:text-light font-bold' : 'text-neutral-700 font-medium dark:text-neutral-400'
        )}
        href={item.url}
      >
        {item.title}
